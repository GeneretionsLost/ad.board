...
@section('title', 'Объявления на модерацию')
    ...
    @foreach($products as $product)
        <main class="add-page">
            <div class="content-wrapper">
                ...
                <div class="date text-muted small">Дата добавления: {{ $product->created_at->isoFormat('D MMMM YYYY') }}</div>

                <div class="confirmation-buttons d-flex gap-2">
                    <form action="{{route('post.update',['post'=>$product->id])}}" method="post">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="btn btn-success">Подтвердить</button>
                    </form>
                    <form action="{{route('post.destroy',['post'=>$product->id])}}" method="post">
                        @csrf
                        @method('DELETE')
